...
import { TravelDetails, TravelSuggestion } from "@/api/travel/types"
...
import { getTravelDetails, getTravelSuggestions } from "@/api/travel"
...
import { getTravelReviews } from "@/api/review/travel_review"

interface TravelCardProps extends DetailedHTMLProps<HTMLAttributes<HTMLElement>, HTMLElement> {
  id: string
}

export default function TravelPresentation({ id, className, ...props }: TravelCardProps) {
  ...
  } = useQuery<any, CustomAxiosError, TravelReview[]>({
    queryFn: () => getTravelReviews(id),
  ...
  } = useQuery<any, CustomAxiosError, TravelSuggestion[]>({
    queryFn: () => getTravelSuggestions(data?.travel_agency._id ?? "", data?._id ?? ""),
    queryKey: ["travelSuggestions", data?.travel_agency._id ?? "", data?._id ?? ""]
  })
    ...
    <article className={cn("font-dm-sans", className)} {...props}>
      <section className="page-container page-container-sm">
        <h1 className="text-5xl font-medium">
          Let&apos;s visit {data.destination.split(",")[0] ?? data.destination} with {data.travel_agency.name} !
        </h1>
        <div className="flex items-center gap-10 pt-6 text-sm">
          <div className="flex items-center gap-2 font-medium">
            <StarIcon className="text-[#FFC542]" />
            <span>
              {data.travel_agency.rating}{" "}
              <span className="text-primary-gray">({data.travel_agency.reviews_count} reviews)</span>
            </span>
          </div>
        ...
        <div className="flex items-center gap-12 pt-8">
          <BadgesSection experiences={data.experiences} />
          <Review reviews={data.travel_agency.rating} />
        </div>
      </section>
      ...
      <section className="page-container page-container-sm flex items-start justify-between gap-10">
        <div className="max-w-[46rem] grow">
          ...
          <AgencyCard agency={data.travel_agency} />
          <p className="pt-10">{data.description}</p>
          <div className="pt-14">
            <h5 className="text-xl font-bold">Additional features</h5>
      ...
      <Plan steps={data.plan} className="page-container page-container-sm mt-20" title={"Trip Plan!"} />
      ...
      {!isErrorSuggestions && (
        <SuggestionsSection
          link="/travels"
          title="You may be interrested in these trips too!"
